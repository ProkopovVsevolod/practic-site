...
public class SecurityConfig {
  ...
  private void authorizeRequest(AuthorizeHttpRequestsConfigurer<HttpSecurity>.AuthorizationManagerRequestMatcherRegistry registry) {
    registry.requestMatchers("/api/v1/users/signup").permitAll();
    registry.requestMatchers("/api/v1/users/login").permitAll();
    registry.requestMatchers("/api/v1/users/refresh").permitAll();
    registry.requestMatchers("/api/v1/users/logout").permitAll();
  }
}
